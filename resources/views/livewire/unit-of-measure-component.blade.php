<div class="">
                ...
                <div class="col-span-12">
                    <div class="card border-0 overflow-hidden">
                        ...
                        <div class="card-header flex items-center justify-between">
                            <h6 class="card-title mb-0 text-lg">Measures</h6>
                            <input 
                                type="text" 
    ...
    <tbody>
        @forelse ($measures as $index => $measure)
            <tr class="border-t border-neutral-200 dark:border-neutral-600">
                <!-- Checkbox + Serial -->
                <td class="px-4 py-2">
                    <div class="form-check style-check flex items-center">
                        <input class="form-check-input" type="checkbox" value="{{ $measure->id }}">
                ...
                <td class="px-4 py-2 text-left">
                    <span class="{{ $measure->active 
                        ...
                        {{ $measure->active ? 'Active' : 'Inactive' }}
                    </span>
                </td>
                ...
                <td class="px-4 py-2 text-left space-x-2">
                    <button 
                        wire:click="edit({{ $measure->id }})"
                        class="w-8 h-8 bg-green-600 text-white rounded-full inline-flex items-center justify-center hover:bg-green-700">
                        <iconify-icon icon="lucide:edit"></iconify-icon>
                    </button>
                    <button 
                        wire:click="delete({{ $measure->id }})"
    ...
    <div class="text-sm text-gray-600">
        Showing 
        <span class="font-medium">{{ $measures->firstItem() }}</span> 
        to 
        <span class="font-medium">{{ $measures->lastItem() }}</span> 
        of 
        <span class="font-medium">{{ $measures->total() }}</span> 
        entries
    </div>

    {{-- Pagination --}}
    <div>
        {{ $measures->onEachSide(1)->links('vendor.pagination.tailwind') }}
    </div>
</div>
            ...
            <div class="col-span-12">
                <div class="card border-0">
                    <div class="card-header">
                        <h5 class="text-lg font-semibold mb-0">Add measure</h5>
                    </div>
                    <div class="card-body">
                        <form wire:submit.prevent="save" class="grid grid-cols-12 gap-4">
                            ...
                            <div class="md:col-span-6 col-span-12">
                                <label for="measureName" class="block text-sm font-medium text-gray-700 mb-2">Measure Name</label>
                                <input id="measureName" name="measureName" type="text" wire:model.defer="name" placeholder="Enter measure name"
